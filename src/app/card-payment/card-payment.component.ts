...
export class CardPaymentComponent implements OnInit, OnDestroy {
  ...
  buildForm() {
    this.paymentForm = this.formBuilder.group({
      ...
      ccHolder: ['', [Validators.required,Validators.minLength(1),Validators.pattern('^[A-Za-z][A-Za-z -]*$')]],
  ...
  submitForm() {
    if (this.paymentForm.status === 'VALID') {
      ...
      const paymentFormData = {
        ...
        cardHolder: this.paymentForm.get('ccHolder').value,
      ...
      this.submitted = true;
      this.errorMessage = "Please fill in your credit card details";
    }
  }
